function [Fitdata,Fitdatagof,Fitdataoutput,Fitdatayy,Fitdataresid,yyy,yyy2,yyy3,xxx,MDCM,MaxDCM,h]= dataanalyzer3 (xarray1,ymatrix,ET,sdmatrix,ETnew,CMILP,CSDLP)
...
[fresult,gof,output,yy,resid,fresult3,gof3,output3,weightvector]= powerfitAMP(xarray1,yarray1,sdarray1);
Fitdata{i}=[fresult];
Fitdata2{i}=[fresult3];
Fitdatagof{i}=[gof];
...
xxx=logspace(log10(min(xarray1)),log10(max(xarray1)));
...
kk=semilogx(xxx,yyy2(i,:),xxx,yyy3(i,:),xarray1,yarray1,'ks',[MDC, MDC],[-100,300 ],'b--',[MaxDC, MaxDC],[-100,300],'b--');
set(kk,'LineWidth',2);
...
legend ('5 parameter SD Weights','4 parameter','Known Data','location','SouthWest');
end

toc
set(figure(1), 'paperunits', 'inches', 'paperposition', [0 0 18 15])
print(figure(1),'-dtiff','-r300', 'C:\Documents and Settings\carevalos\Desktop\McDevitt Lab\Data Anyalsis Program\AMP\AMP known.tif')
mom;
 pp=-1;
i=numberofexposures;
...
%xlsPasteTo('Data.xlsx','Sheet1',1000, 1000,'A1')
 ...
 MDCM(i)=MDC;
MaxDCM(i)=MaxDC;
[unkconc]=unknownfinderAMP(Fitdata{i},Fitdata2{i},unk,sd,ETnew(i),xarray1,yarray1,sdarray1,p+1,CMILP(i),CSDLP(i),h); 
