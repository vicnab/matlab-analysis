function [unk]=unknownfinder(fit,fit2,y,sd,ETT,xarray1,yarray1,sdarray1,p,CMILP,CSDLP,h,MDCc,MaxDCc)
...
xxx2=logspace(log10(min(xarray1)/10),log10(max(xarray1)*10),1000);

kk=semilogx(xxx2,fit(xxx2),xxx2,fit2(xxx2),xxx(a),yy(a),'bx',xarray1,yarray1,'ks',median(xxx(a)),median(yy(a)),'ro',[MDCc, MDCc],[0,300 ],'b--',[MaxDCc, MaxDCc],[0,300],'b--',[median(xxx(a)), median(xxx(a))],[0, median(yy(a))],'r--',[xxx(1), median(xxx(a))],[median(yy(a)), median(yy(a))],'r--');
set(kk,'LineWidth',2);
hh=legend ('5 parameter SD Weights','4 parameter','95% percentile unknown range','Known Data','Unknown Value','location','NorthEast');
set(hh,'FontSize',22);
format short
...
print(figure(p),'-dtiff','-r300', ['C:\Documents and Settings\carevalos\Desktop\McDevitt Lab\Data Anyalsis Program\Dia\Diaunknown',num2str(p-1),'.tif']);