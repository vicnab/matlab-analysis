function [fresult,gof,output,yy,resid,fresult3,gof3,output3,weightvector]=powerfit4dia(xvalues,yvaluesarray,sdb,C)
format long
fresult=1;
gof=1;
output=1;
...
mymodel2 = fittype('v+(a-v)/((1+(x/c)^b)^z)');
...
set(opts,'normalize','off','MaxIter',1*10^8,'MaxFunevals',100000,'Startpoint',[1.1*max(yvaluesarray) 1 1 max(yvaluesarray) .5],'Lower',[.9*max(yvaluesarray) -1 0 0 0],'Upper',[260 1200 2 2*C 10],'TolFun',10^-12,'TolX',10^-12,'Weights',weightvector);
...
set(opts3,'normalize','off','MaxIter',1*10^8,'MaxFunevals',100000,'Startpoint',[min(yvaluesarray) 1 1 max(yvaluesarray)],'Lower',[min(yvaluesarray) -1 0 0],'Upper',[260 1200 2 2*C],'Weights',weightvector);
