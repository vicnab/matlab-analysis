...
maxima = [];
 ...
 for bead = 1:20;
     ...
     average(bead,1) = line_prof_averages(xi(1)-1, xi(2)-1, min(yrange)-1, max(yrange)-1, double(redimg)); 
     average(bead,2) = line_prof_averages(xi(1)-1, xi(2)-1, min(yrange)-1, max(yrange)-1, double(greenimg)); 
     average(bead,3) = line_prof_averages(xi(1)-1, xi(2)-1, min(yrange)-1, max(yrange)-1, double(blueimg)); 
     average(bead,4) = line_prof_averages(xi(1)-1, xi(2)-1, min(yrange)-1, max(yrange)-1, double(grayimg)); 
 end
