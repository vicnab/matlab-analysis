function [row] =  findmissing_demo(row,topleft,bottomright, img, meanrad,debugon) 
if (nargin<6)
    debugon = 0;
end
[m n colors] = size(img);
...
if(~b1)
    ...
    elseif(b4 & b2)
        ...
        bead1 = findnewcenter(img, centeraprx,meanrad,1);
        if(isempty(bead1))
            if(debugon)
             disp('just an approximation for bead1')
            end
...
function newcenter =  findnewcenter(img, center,meanrad, demo)
        if(nargin<4)
            demo = 0;
        end
        ...
        if(demo)
            ...
            h = text(100,100, sprintf('Mean Value is %3.0f', middle));
            set(h,'color', [1 0 0]);
            h = text(100,110, sprintf(' STD is %3.3f', vari));
            set(h,'color', [1 0 0]);
            pause;
        end
        img(find(img > middle + 2 * vari)) = 255;
        ...
        newcenter = [circen cirrad];
         ...
         if(demo)
             ...
             figure(2); DrawCircle(newcenter(1)-xbound, newcenter(2)-ybound, newcenter(3), 20, '-r', 2); hold off;
             pause(1);
         end
