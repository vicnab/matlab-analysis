...
pause();
if(length(row1)<5)
     row1 = findmissing_demo(row1,topleft, bottomright, calimg, meanrad,0);
end
if(length(row2)<5)
     row2 = findmissing_demo(row2, topleft, bottomright, calimg, meanrad,0);
end
if(length(row3)<5)
     row3 = findmissing_demo(row3, topleft, bottomright, calimg, meanrad,0);
end
if(length(row4)<5)
     row4 = findmissing_demo(row4, topleft, bottomright, calimg, meanrad,0);
end
...
hold on;
for k = 1:length(centers)
    DrawCircle(centers(k,1), centers(k,2), centers(k,3), 20, 'xr', 2);
end
